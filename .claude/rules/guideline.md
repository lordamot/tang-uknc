...
- **`tang/test003.gprj` is the source of truth for what is built.**  About
  forty files are in it; roughly as many `.v` files under `tang/src/` are
  not.  Check the project file before editing anything, or you will fix a
  bug in a file the synthesiser never reads.  `.claude/docs/fpga.md` has
  the live/dead list.
- Never add a file to `tang/src/` without adding it to the `.gprj`, and
  never delete a "dead" file on the strength of it being dead - several are
  earlier revisions the author still refers to.
