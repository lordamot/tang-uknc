...
- **The AY chip select is not at the address its comment names.**
  ...
  **Since v2.0.0 (3 Sep 2026) every add-on is behind an OSD switch and a
  device that is off is not acknowledged at all** - `aberrant.v` per
  chip (`'1'` `'2'` `'3'`), `covox.v` (`'c'` bit 0), the printer-port
  DAC on `vp1_120.v`'s port A (`'c'` bit 1, mixed in `top.v`, the port
  itself always answers), `vp1-128fdd.v` (`'f'`), the IDE cartridge
  (`'e'` ANDed into `hdd_present`), `kakave.v` per word (`'u'` `'t'`);
  the beeper (`'b'`) only leaves the mixer.  A decode that ignores its
  enable, or a testbench that instantiates one of these without it, is
  the trap now: "the Covox is absent" on a board may be the OSD's
  default (Off), and the defaults are a plain machine with the three AYs.
...
- **A menu value needs three edits, not one**: the letter in the form
  ...
  Since v2.0.0 (3 Sep 2026) the menu is main / Hardware / Aberrant /
  FDD controller / HDD controller / RTC clock / Misc plus an About page,
  `.claude/docs/mcu.md` has the letters, and a form returns to the entry
  that opened it **by form number** (`menu_parent_entry()`), so the
  `"parent|entry"` in a title is only a fallback and adding an entry no
  longer means renumbering - that is what broke every time before.  The
  **main form is built at run time** (`menu_uknc_main()`): "HDD0:" is
  there only while the HDD controller is on, so never hardcode a
  main-form entry number.  `make menu-test` walks every form on the
  host and writes each screen as a PNG under `build/menu/` - it is the
  only way to see whether a label and its value fit the 128 pixels, and
  "HDD image format" beside "Inversed" did not.  The old `'P'` "Disk
  prot." sent a six-entry list INDEX that the FPGA read as a bitmask
  ("2:" protected drives 0 and 1); it is four letters now.  "Run SAV:"
  is the one fileselector that mounts nothing: it browses through
  `SDC_SLOT_SAV` and hands the file to `rt11sav.c`, which writes the
  card - `make sav-test` runs that code on the host, and it is the only
  place it can be watched.  **`sdc.c`'s FatFs
...
- **A value sysctrl samples out of the PPU domain has 20 ns, so it must
  come from a flop.**  The clock read-back (SYS command 6, v2.0.0) takes
  `kakave.v`'s counters onto clk_25 and the weekday was ~30 ns of
  arithmetic behind the year register: the first v2.0.0 layout failed
  the timing gate on exactly that, three endpoints, and a registered
  `dow_r` fixed it.  Anything new that `sysctrl.v` reads from the PPU
  side gets a register at the source.
